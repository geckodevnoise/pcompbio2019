...
%this function creates a binary disk
%with a threshold equal to mean-sigma
%rotated to match the head picture

%before being saved as 'gecko682_4stats_204x3PPI.mat'
%they were NOT rotated

%%% gecko
%%% 681, 682, 731, 732, 735, 763, 772, 773
%%%
function [binary_image,binary_imageROI]=CreateDiskShapedBinaryHead_July_2016(gecko,f);
...
cutROI=ROI_image(a:b,c:d);
%rotated=imrotate(cut,f_rot,'crop');
rotated=imrotate(cut,0,'crop');%no rotation
rotatedROI=imrotate(cutROI,0,'crop');%no rotation
m=size(rotated);
a=m(1,1);
b=m(1,2);
...
binary_image=rotated;
binary_imageROI=rotatedROI;
