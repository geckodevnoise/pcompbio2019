...
function Figure4_genotype_matches_Aug11_res61_color(new_grids_switch)
...
for k=1:8
    ...
    if max(gecko==[763 735 773 681])
        pattern=(distance_grid<50).*(distance_grid>0)*(k+1);
    else
        pattern=(distance_grid<50).*(distance_grid>0)*(k+1);
    end
...
fu_low=min(find(vary_fu_g>=.81));
fu_high=min(find(vary_fu_g>=.859));
spacer=ceil((fu_high-fu_low)/5);
Xticks=(fu_low:spacer:fu_high); 
set(gca,'XTick',(Xticks))
set(gca,'XTickLabel',vary_fu_g(Xticks))

FT_low=min(find(vary_FT_g>=.94));
FT_high=max(find(vary_fu_g<=1.85));
spacer=ceil((FT_high-FT_low)/7);
Yticks=(FT_low:spacer:FT_high); %resolution 61 (vary_FT_g=0.80:vft:1.85)
...
fu_low=min(find(vary_fu_g>=.805));
fu_high=min(find(vary_fu_g>=.845));
xlim([fu_low,fu_high])%resolution 61

FT_low=min(find(vary_FT_g>=.9925));
FT_high=max(find(vary_fu_g<=1.85));
ylim([FT_low,FT_high])%resolution 61
set(gca,'FontSize',18)
xlabel('Activation Rate \it{f_u}')
...
for i=1:8
    ...
    if gecko==731
        text(i+2*sk,j+3*sk,num2str(gecko),'Color',[0 0 0],'FontSize',14)
    elseif gecko==735
        text(i+1*sk,j+3*sk,num2str(gecko),'Color',[0 0 0],'FontSize',14)
    else
        text(i-4*sk,j+3.5*sk,num2str(gecko),'Color',[0 0 0],'FontSize',14)
    end
        hold on
end
...
fu_low=min(find(vary_fu_g>=.802));
fu_high=fix((.862-.8)/vfu+1);
FT_low=min(find(vary_FT_g>=.835));
FT_high=max(find(vary_fu_g<=1.85));
xlim([fu_low,fu_high])%resolution 61
ylim([FT_low,FT_high])%resolution 61
