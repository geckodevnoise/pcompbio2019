...
for i=1:size(vary_fu_g,2)
    ...
    for j=1:v:size(vary_FT_g,2)
        ...
        ColorRed=(distance_grid_731(j,i)<=50);
        ColorPink=[(distance_grid_731(j,i)<outer).*(distance_grid_731(j,i)>50)];
        ColorBlue=(distance_grid_763(j,i)<=50);
        ColorSky=[(distance_grid_763(j,i)<outer).*(distance_grid_763(j,i)>50)];
        
        if max([ColorRed,ColorPink,ColorSky,ColorBlue])
            ...
            %LeftPanel
            for k=1:num_spots
                scatter(fu+Dfu*rand(1)*a,ft+Dft*rand(1)*a,size_spots,LeftPanel,'s','filled')
                hold on
                scatter(fu+Dfu*rand(1)*a,ft+Dft+Dft*rand(1)*a,size_spots,LeftPanel,'s','filled')
            end
            %RightPanel
            for k=1:num_spots
                scatter(fu+Dfu+Dfu*rand(1)*a,ft+Dft*rand(1)*a,size_spots,RightPanel,'s','filled')
                hold on
                scatter(fu+Dfu+Dfu*rand(1)*a,ft+Dft+Dft*rand(1)*a,size_spots,RightPanel,'s','filled')
            end    
        end
    end
end
