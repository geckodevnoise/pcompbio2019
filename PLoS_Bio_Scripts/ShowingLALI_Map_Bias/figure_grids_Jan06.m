...
function figure_grids_Jan06

vary_fu_g=.8:.001:.85;%to match the grid from figure_genotype_clouds_all_Jan02
vary_FT_g=0:.02:3;%to match the grid from figure_genotype_clouds_all_Jan02

c=[.7 .7 0];
blue_lines=[0 .5172 .5862];
blue_lines=[.1 .5 1];
red_lines=[1 0 0];
...
for i=1:size(GenotypeGrid_fu,2)
    for j=1:size(GenotypeGrid_FT,2) 
            i2=min(find(abs(vary_fu_g-GenotypeGrid_fu(i))==min(abs(vary_fu_g-GenotypeGrid_fu(i)))));
            j2=min(find(abs(vary_FT_g-GenotypeGrid_FT(j))==min(abs(vary_FT_g-GenotypeGrid_FT(j)))));
        scatter(i2,j2,'filled','MarkerFaceColor','r')
        hold on
    end
end
figure(f)
for i=1:(size(GenotypeGrid_fu,2)-1)
    for j=1:size(GenotypeGrid_FT,2) 
        line([GenotypeGrid_fu(i),GenotypeGrid_fu(i+1)],[GenotypeGrid_FT(j),GenotypeGrid_FT(j)],'Color',red_lines,'LineWidth',2)
        hold on
    end
end
for i=1:size(GenotypeGrid_fu,2)
    for j=1:(size(GenotypeGrid_FT,2)-1) 
        line([GenotypeGrid_fu(i),GenotypeGrid_fu(i)],[GenotypeGrid_FT(j),GenotypeGrid_FT(j+1)],'Color',blue_lines,'LineWidth',2)
        hold on
    end
end
...
for i=1:(size(GenotypeGrid_fu,2)-1)
    for j=1:size(GenotypeGrid_FT,2) 
        line([PhenotypeMatrix(i,j,1),PhenotypeMatrix(i+1,j,1)],[PhenotypeMatrix(i,j,2),PhenotypeMatrix(i+1,j,2)],'Color',red_lines,'LineWidth',2)
        hold on
    end
end
for i=1:(size(GenotypeGrid_fu,2))
    for j=1:(size(GenotypeGrid_FT,2)-1) 
        line([PhenotypeMatrix(i,j,1),PhenotypeMatrix(i,j+1,1)],[PhenotypeMatrix(i,j,2),PhenotypeMatrix(i,j+1,2)],'Color',blue_lines,'LineWidth',2)
        hold on
    end
end
